...
describe("List medics", () => {
  ...
  it("should be able to list all medics", async () => {
    ...
    const response = await request(app).get("/medics").set({
      Authorization: `Bearer ${token}`
    })
    ...
    expect(response.body[0].name).toEqual("Medico 1")
    expect(response.body[1].name).toEqual("Medic Supertest")
  })
})
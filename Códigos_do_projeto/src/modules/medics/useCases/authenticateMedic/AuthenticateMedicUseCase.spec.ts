...
import { CreateMedicUseCase } from "../createMedic/CreateMedicUseCase"
...
describe("Authenticate Medic", () => {
  beforeEach(() => {
    ...
    authenticateMedicUseCase = new AuthenticateMedicUseCase(medicsRepositoryInMemory, medicsTokensRepositoryInMemory, dateProvider)
  ...
  it("should not be able to authenticate an non-existing medic", async () => {
    await expect(authenticateMedicUseCase.execute({
      cpf: "000",
      password: "0"
    })

...
class MedicsRepository implements IMedicsRepository {
  ...
  async create({ name, nationality, ethnicity, crm, cpf, password, marital_status, birth_date, address, city, state, gender, especialization, phone_number, id }: ICreateMedicDTO): Promise<Medic> {
  ...
  async updateMedic(id: string, data: IUpdateMedicDTO): Promise<void> {
  ...
  async getAll(): Promise<Medic[]> {
    const medics = await this.repository.find({
      select: ["id", "name", "nationality", "ethnicity", "crm", "cpf", "marital_status", "birth_date", "address", "city", "state", "gender", "especialization", "phone_number"]
    })
    return medics
  }

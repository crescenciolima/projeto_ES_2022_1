...
describe("Delete a patient", () => {
  ...
  it("should be able to delete a patient", async () => {
    ...
    await request(app).post("/patients")
      .send({
    ...
    const patients = await request(app).get("/patients").set({
      Authorization: `Bearer ${token}`
    })

    const response = await request(app).delete(`/patients/${patients.body[0].id}`).set({
      Authorization: `Bearer ${token}`
    })
